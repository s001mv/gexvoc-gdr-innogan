...
namespace GEXVOC.Informes
{
    public partial class InfLibroExplotacion : GEXVOC.Core.Controles.ctlInforme
    {
        ...
        public override CrystalDecisions.CrystalReports.Engine.ReportDocument Generar()
        {
            ...
            List<Animal>lstAnimales=Animal.Buscar(explotacion.Id,Generic.ControlAIntNullable(cmbEspecie),true);
            foreach (Animal item in lstAnimales)
            {
                if (!(item.Externo ??false))//Solo se representan en el libro de la explotacion los animales que no tengan externo=true o lo tengan a null.
                {
