	...
	public class FloatingWindow: NativeWindow, IDisposable
	{
		...
		protected virtual void PaintShadow(PaintEventArgs e)
		{
			if (this.hasShadow)
			{
				...
				path1.StartFigure();
				path1.AddRectangle(this.ShadowRectangle);
				path1.CloseFigure();
				using (PathGradientBrush brush2 = new PathGradientBrush(path1))
				{
					brush2.CenterColor = color1;
					brush2.CenterPoint = new Point(this.ShadowRectangle.X+this.ShadowRectangle.Width/2,
						this.ShadowRectangle.Y+this.ShadowRectangle.Height/2);
					ColorBlend cb = new ColorBlend();
					cb.Colors = new Color[] {color1, color2};
					...
					e.Graphics.FillRectangle(brush2, this.ShadowRectangle);
				}
				path1.Dispose();
		...
		protected virtual void PerformPaint(PaintEventArgs e)
		{
			...
			using (SolidBrush brush1 = new SolidBrush(Color.FromArgb(255, this.backColor)))
			{
				if (!this.minimized)
				{
				...
				using (SolidBrush brush2 = new SolidBrush(ControlPaint.Dark(this.backColor)))
				{
					if (this.minimized)
					{
					...
					if (this.text.Length>0)
					{
						using (StringFormat sf = new StringFormat(StringFormat.GenericTypographic))
						{
							...
							sf.FormatFlags = sfg;
							using (SolidBrush brush3 = new SolidBrush(ControlPaint.Light(this.foreColor)))
							{
								e.Graphics.DrawString(this.text, Control.DefaultFont, brush3, new Rectangle(rect.X+3, rect.Y, rect.Width-6, rect.Height), sf);
							}
				...
				using (Pen pen1 = new Pen(this.borderColor))
				{
					...
					if (!this.minimized)
					{
						...
						pen1.Color = ControlPaint.Light(this.borderColor);
						...
						this.resizeR = new Rectangle(rect2.Right-9, rect2.Bottom - 9, 9, 9);
					}
				}
			}
		}
		...
		{
			if (!this.closeRect.Contains(this.lastMouseDown) && 
				!this.expandRect.Contains(this.lastMouseDown))
			{
		...
		private bool PerformWmNcHitTest(ref Message m)
		{
			...
			if (this.expandRect.Contains(point1.x, point1.y))
			{
				return false;
			}
			if (this.closeRect.Contains(point1.x, point1.y))
			{
				return false;
			}
			...
			else
			{
				switch (num1)
				{
					...
					case 0x202: // WM_LBUTTONUP
					{
						...
						if (this.onMouseUp)
						{
							if (this.closeRect.Contains(p))
							{
								this.Hide();
								return;
							}
							if (this.expandRect.Contains(p))
							{
								if (this.minimized)
									this.Maximize();
								else
